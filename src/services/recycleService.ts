...
// ==================== POINT WALLET & LEDGER ====================
export const getPointWallet = async (): Promise<{
  success: boolean;
  data?: PointWalletUI;
  message?: string;
}> => {
  try {
    const authHeaders = await getAuthHeaders();
    const res = await fetch(getApiUrl(`/points/wallet?t=${Date.now()}`), {
      headers: authHeaders,
    });
    return await readApiJson(res);
  } catch (err: any) {
    console.error("getPointWallet error:", err);
    return { success: false, message: err?.message || "Gagal memuat saldo point" };
  }
};

export const getPointLedger = async (params?: {
  type?: string;
  page?: number;
  limit?: number;
}): Promise<{
  ...
  try {
    ...
    const url = getApiUrl(`/points/ledger${query.toString() ? `?${query.toString()}` : ""}`);
    const res = await fetch(url, { headers: authHeaders });
    return await readApiJson(res);
  } catch (err: any) {
    console.error("getPointLedger error:", err);
    return { success: false, message: err?.message || "Gagal memuat buku tabungan" };
  }
};

// ==================== REDEMPTIONS ====================
export const createPointRedemption = async (
  payload: {
  ...
  idempotencyKey?: string
): Promise<{ success: boolean; data?: PointRedemptionUI; message?: string }> => {
  try {
    const authHeaders = await getAuthHeaders();
    const res = await fetch(getApiUrl("/points/redemptions"), {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...authHeaders,
    ...
    console.error("createPointRedemption error:", err);
    return { success: false, message: err?.message || "Gagal memproses penukaran point" };
  }
};
...
export const getCustomerRedemptions = async (
  ...
  try {
    const authHeaders = await getAuthHeaders(customerId);
    const res = await fetch(getApiUrl(`/points/redemptions/customer/${customerId}?t=${Date.now()}`), {
      headers: authHeaders,
    });
    return await readApiJson(res);
  } catch (err: any) {
    console.error("getCustomerRedemptions error:", err);
    return { success: false, data: [], message: err?.message || "Gagal memuat riwayat penukaran" };
  }
};
...
export const getAvailableVouchers = async (
  service?: string
): Promise<{ success: boolean; data: VoucherUI[]; message?: string }> => {
  try {
    const query = service ? `?service=${encodeURIComponent(service)}` : "";
    const res = await fetch(getApiUrl(`/vouchers${query}`));
    return await readApiJson(res);
  } catch (err: any) {
    console.error("getAvailableVouchers error:", err);
    return { success: false, data: [], message: err?.message || "Gagal memuat katalog voucher" };
  }
};

export const getMyVouchers = async (): Promise<{
  success: boolean;
  data: VoucherUI[];
  message?: string;
}> => {
  try {
    const authHeaders = await getAuthHeaders();
    const res = await fetch(getApiUrl(`/vouchers/my?t=${Date.now()}`), {
      headers: authHeaders,
    });
    return await readApiJson(res);
  } catch (err: any) {
    console.error("getMyVouchers error:", err);
    return { success: false, data: [], message: err?.message || "Gagal memuat voucher saya" };
  }
};
